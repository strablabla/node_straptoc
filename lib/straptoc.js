...
function initMiniTable(tableId){
    ...
    var tableSettings = { fontSize: 14, headerFontSize: 14, headerBold: true }
    var saveTimer = null
    ...
    function showColStatsPopup($cell, colIdx){
        ...
        $apply.click(function(){
            ...
            for (var k in selection){
                var parts = k.split('_')
                if (parseInt(parts[1]) === colIdx) selectedRows.push(parseInt(parts[0]))
            }
            if (selectedRows.length > 0){
                ...
                for (var r = 1; r < tableData.length - 1; r++){
                    var v = parseFloat(tableData[r][colIdx])
                    if (!isNaN(v)) vals.push(v)
                }
            }
            if (vals.length === 0){ $popup.remove(); return }
            var n = vals.length
            var mean = vals.reduce(function(a,b){ return a+b }, 0) / n
            var parts = []
            if ($meanCb.is(':checked')){
                parts.push('μ=' + (+mean.toFixed(4)))
            }
            if ($stdCb.is(':checked')){
                var variance = vals.reduce(function(a,b){ return a + (b - mean) * (b - mean) }, 0) / n
                var std = Math.sqrt(variance)
                parts.push('σ=' + (+std.toFixed(4)))
            }
            if (parts.length === 0){ $popup.remove(); return }
            var lastRow = tableData.length - 1
            tableData[lastRow][colIdx] = parts.join(' ')
            render(); save()
            $popup.remove()
        })
    ...
    function render(){
        ...
        $settingsBtn.click(function(e){
            ...
            var inputCss = {'width': '30px', 'font-size': '11px'}
            ...
            // Header bold
            var $boldInput = $('<input/>').attr('type', 'checkbox').prop('checked', tableSettings.headerBold !== false)
            $boldInput.on('change', function(){
                tableSettings.headerBold = $(this).is(':checked')
                $container.find('.mini-table tr').first().find('td').css('font-weight', tableSettings.headerBold ? 'bold' : 'normal')
                debounceSave()
            })
            ...
            $popup.append($close)
                  ...
                  .append(makeRow('Bold header', $boldInput))
            $('body').append($popup)
        ...
        for (var r = 0; r < tableData.length; r++){
            ...
            for (var c = 0; c < tableData[r].length; c++){
                var fs = (r === 0 ? tableSettings.headerFontSize : tableSettings.fontSize)
                ...
                var $td = $('<td/>').attr('contenteditable', 'true')
                                    ...
                                    .css('font-size', fs + 'px')
                if (r === 0) $td.css('font-weight', tableSettings.headerBold !== false ? 'bold' : 'normal')
