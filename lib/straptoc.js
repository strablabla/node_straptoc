...
function initMiniTable(tableId){
    ...
    function render(){
        ...
        // title
        var $title = $('<div/>').addClass('mini-table-title').text(tableId)
        ...
        var $colBtns = $('<div/>').css({'display': 'flex', 'flex-direction': 'column', 'margin-left': '5px', 'gap': '3px', 'align-self': 'flex-start'})
        ...
        var $tdBtns = $('<td/>').css({'border': 'none', 'padding': '3px 0', 'text-align': 'right'})
    ...
    function save(){
        sock.emit('save_table_data', JSON.stringify({ id: tableId, data: tableData }))
    }
    ...
    sock.on('table_data_' + tableId, function(json){
        ...
        if (data && data.length) tableData = data
        render()
    })
    }) // end waitForSocket
}
