    ...
    */
    ...
    $("p, li").each(function(){
        if ($(this).text().match(/^\$vid/)) {  // detect list of local videos
           ...
           // Wrap parent's direct text nodes in inline-block span for click targeting
           var parentLi = ulvid.closest('li')
           if (parentLi.length) {
               parentLi.contents().filter(function(){ return this.nodeType === 3 && $.trim(this.nodeValue) !== '' }).wrap('<span class="vid-group-label"/>')
           }
        }   // end if regexp
    })   // end each p, li
    ...
    $('a').each(function(){            // modifying videos for permitting folded list mechanism.
            if ($(this).text().match(';;')){
             ...
             if ($(this).text().match('%%')){    // modifying local videos for permitting folded list mechanism.
                 ...
                 var strapvidParent = $(this).parent().parent().parent()
                 strapvidParent.addClass('strapvid_inside')
                 $(this).parent().replaceWith(ulvid) // replace <a> with a <ul> containing <a>
                 // Wrap parent's direct text nodes in inline-block span for click targeting
                 strapvidParent.contents().filter(function(){ return this.nodeType === 3 && $.trim(this.nodeValue) !== '' }).wrap('<span class="vid-group-label"/>')
              } // end if
