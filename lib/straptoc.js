...
var maketoc = function(){
    ...
    * Lists
        ...
        * Alt + Q : active/désactive le mode deep fold (déplie/replie toute la hiérarchie d'un clic)
    * Insert docs
    ...
    * Lists
        * Alt + Q : deep fold mode (déplie/replie toute la hiérarchie d'un clic sur [+]/[-])
    * Handle windows
        * Esc + r : resize
...
//===================================================================== Regexp for parameters
    ...
    */
    ...
    var reg_id = /--[\w:]*--/; //regexp for identity (includes : for namespaced tags)
...
//===================================================================== Dictionary for parameters
 ...
 //===================================================================== Go to top
    ...
    */
    ...
    var scrollTopBtn = $('<a/>').attr('href','#top').addClass("scrollToTop").attr('title','go to top')
    var topIcon = $('<span/>').attr('id','gotop').text('▲')
    scrollTopBtn.append(topIcon)
 ...
 //===================================================================== Go to bottom
    ...
    */
    ...
    var scrollBottomBtn = $('<a/>').attr('href','#bottom').addClass("scrollToBottom").attr('title','go to bottom')
    var bottomIcon = $('<span/>').attr('id','gobottom').text('▼')
    scrollBottomBtn.append(bottomIcon)
    $('body').append(scrollBottomBtn)
...
//===================================================================== Carousels
 ...
 //===================================================================== Simple bash code
    ...
    */
    ...
    $("p, li").each(function(){
    var ht = $(this).html()
    // Only process ``` if it's in direct content, not in nested <ul> children
    var ulIndex = ht.indexOf('<ul')
    var backtickIndex = ht.indexOf('```')
    // Skip if ``` is not present or is inside nested list
    if (backtickIndex == -1 || (ulIndex != -1 && backtickIndex > ulIndex)) {
        return
    }
    var newtxt = ht.replace('```',' ').replace(/<br\s*\/?>/gi,'\n').trim()
    var pre = $('<pre/>').append(
  ...
  */
  ...
  $("p, li").each(function(){
    // A markdown hard break (two or more spaces at the end of a source line)
    // renders as <br> and swallows the newline. Without this, a '+++ root' line
    // ending in blanks came back glued to the <a> of the next source line, and
    // the whole markup ended up inside the root path.
    var htm = $(this).html().replace(/<br\s*\/?>/gi, '\n')
    var txt = htm.split('\n')[1] || ' '
    if (txt.match(/\+\+\+\.*/)) {
...
//===================================================================== list of local videos
    ...
    /*
    Takes all the addresses after $vid and insert in list with lazy loading.
    Videos are only loaded when user clicks [+] to expand.
    Compatible with play_list_vid.html playlist system.
    */

    $("p, li").each(function(){
        if ($(this).text().match(/^\$vid/)) {  // detect list of local videos
           var ulvid = $('<ul/>').addClass('vid-lazy-list')
           $(this).children('a').each(function(){
                var videoPath = $(this).attr('href')
                var videoName = $(this).text().split('%%')[0].trim().split('/').pop() || videoPath.split('/').pop().replace(/\.(mp4|webm|ogg|mkv|avi)$/i, '')

                // Create lazy loading structure
                var viewerId = 'vid-lazy-' + Date.now() + '-' + Math.floor(Math.random() * 1000)
                var div_format = $('<div/>').text(videoName).addClass('embed_video vid-lazy-title')
                var a_deploy = $('<a href="javascript:void(0)"/>').text(' [+]').addClass('vid-lazy-toggle')
                var span_tog = $('<span/>').addClass('tog_elem_vid').append(a_deploy)

                // Create video element without src (lazy - src added on [+] click)
                var video = $('<video/>').attr('data-src', videoPath)
                                         .attr('controls', true)
                                         .css({'width': '100%', 'max-width': '800px'})

                var video_container = $('<div/>').addClass('vid-lazy-container')
                    .attr('id', viewerId)
                    .attr('data-video-path', videoPath)
                    .attr('data-loaded', 'false')
                    .css({'width': '100%', 'margin': '10px 0', 'display': 'none'})
                    .append($('<div/>').addClass('vid-lazy-viewer').append(video))

                // Structure compatible with play_list_vid: li > ul.strapvid > li > div > video
                var inner_li = $('<li/>').append($('<div/>').append(video_container))
                var inner_ul = $('<ul/>').addClass('strapvid').append(inner_li).hide()

                var divformat_plus = div_format.append(span_tog)
                var curr_li = $('<li/>').addClass('fold_li vid-lazy-item')
                curr_li.append(divformat_plus).append(inner_ul)
                ulvid.append(curr_li)
             }) // end each
           $(this).parent().replaceWith(ulvid) // replace whole p or li
        }   // end if regexp
    })   // end each p, li

    // Delegated click handler for $vid lazy loading (works even after DOM insertion)
    $(document).off('click.vidlazy').on('click.vidlazy', '.vid-lazy-toggle', function(e){
        e.preventDefault()
        e.stopPropagation()
        var $this = $(this)
        var $li = $this.closest('.vid-lazy-item')
        var $strapvid = $li.find('.strapvid')
        var $container = $li.find('.vid-lazy-container')
        var $video = $container.find('video')

        if ($this.text() == ' [-]'){
            // Closing - just hide
            $this.text(' [+]')
            $strapvid.slideUp(200)
        } else {
            // Opening - show and lazy load if needed
            $this.text(' [-]')
            $strapvid.slideDown(200)
            $container.show()

            var isLoaded = $container.attr('data-loaded')
            if (isLoaded === 'false') {
                var videoPath = $container.attr('data-video-path')
                // Set the src to actually load the video
    ...
    // Highlight video title in yellow when playing (using capture phase since play/pause don't bubble)
    document.addEventListener('play', function(e) {
        if (e.target.tagName === 'VIDEO') {
            var $video = $(e.target)
            var $li = $video.closest('.vid-lazy-item')
            if ($li.length) {
                $li.find('.embed_video.vid-lazy-title').css('background-color', '#ffff99')
            }
        }
    }, true)
    ...
    document.addEventListener('pause', function(e) {
        if (e.target.tagName === 'VIDEO') {
            ...
            var $li = $video.closest('.vid-lazy-item')
            if ($li.length) {
                $li.find('.embed_video.vid-lazy-title').css('background-color', '')
            }
        }
    }, true)
    ...
    function make_portfolio(){
      ...
      $("p, li").each(function(){
          ...
          if (elem.text().match(/^\$portf/)) {  // detect portfolio
             ...
             elem.replaceWith(divportf) // replace whole p or li with portfolio.
    ...
    */
    ...
    $("p").each(function() {
      if ($(this).html().match(reg_date)){
                $(this).replaceWith(function(){     // Replacing dates with p in date with h3 and
                    var h1prev = $(this).prev("h1").text()
                    var dateh3 = $('<h3/>').text($(this).text()).addClass('date')
                    return dateh3;
                   }) // end replaceWith
               } // end if
      }); // end each
...
//=====================================================================   line symbolic removal.
    ...
    */
    ...
    $("li, p").each(function(){
        var el = $(this)
        // check only direct text nodes (nodeType 3), not nested children
        var hasPercent = false
        el.contents().filter(function(){ return this.nodeType === 3 }).each(function(){
            if ($(this).text().indexOf('%%%') !== -1) { hasPercent = true }
        })
        if (hasPercent) {
            var html = el.html().replace('%%%', '').trim()
...
//===================================================================== mathjax
    ...
    */
    ...
    if (txt.match(/^\$math/)) {
          ...
          $(this).replaceWith('$$' + txt.split('$math')[1] + '$$') // + mathsize +
        }   // end if txt.mtch
    })   // end each p, li
...
//===================================================================== TOC
    ...
    */
    ...
    $('#toc').append($('<h3/>').text("TOC").css({'text-align': 'center', 'margin-top': '0px', 'margin-bottom': '10px'}));
    // $('#toc').append($('<a/>').append($('<span/>').text("[--]").addClass('li_h1')));  // removed: H1 not foldable
    var ul1 = $("<ul/>"); // first levels with class
    $('#toc').append(ul1);
    $('#toc').draggable({   // make the toc draggable with jquery-ui widget draggable.
        // jQuery UI writes an explicit width and height onto the panel while it
        // is being dragged. Left in place they outlive the move and freeze the
        // frame: unfolding a section then grows the list inside a box that
        // cannot follow — measured, 948px of entries in a 144px box — and the
        // table of contents becomes unreadable. Hand the size back on drop.
        stop: function() {
            this.style.width = '';
            this.style.height = '';
        }
    ...
    $('a').each(function(){            // modifying videos for permitting folded list mechanism.
            if ($(this).text().match(';;')){
             ...
             if ($(this).text().match('%%')){    // modifying local videos for permitting folded list mechanism.
                 var tlist = $(this).text().split('%%')[0]
                 var underthis = $('<ul/>').addClass('strapvid').append($('<li/>').append($(this).clone())) // put inside a list, inserted in ulvid
                 var div_tlist = $('<div/>').addClass('embed_video').text(tlist)
                 var ulvid = $('<li/>').append(div_tlist).append(underthis) // <li> with text then clone
                 $(this).parent().parent().parent().addClass('strapvid_inside')
                 $(this).parent().replaceWith(ulvid) // replace <a> with a <ul> containing <a>
              } // end if
    ...
    */
    ...
    $("li").each(function(){    // need to be placed before  $("a").click
        ...
        if(htm.split('\n')[0].search('::')!=-1){
                ...
                var $anchor = $('<a/>').append($('<span/>').text(" [+]").addClass('::'));
                // Check if there's a <p> with :: (happens with code blocks)
                var $pWithColons = $(this).children('p').filter(function() {
                    return $(this).text().indexOf('::') !== -1;
                });
                if ($pWithColons.length > 0) {
                    $anchor.appendTo($pWithColons.first()); // append inside <p> for same line
                } else {
                    $anchor.insertBefore(childr); // normal case: insert before <ul>
                }
            } // end if
        }); // end each
...
//=====================================================================  Folding lists, second step
    ...
    */
    ...
    $('li').each(function (){  // remove the :: (closing list sign) from li text
            var $this = $(this);
            // Check if this li has a span with class :: anywhere inside
            if ($this.find('span.\\:\\:').length > 0){
                // Remove :: from direct text nodes in the li
                $this.contents().filter(function() {
                    return this.nodeType === 3; // text nodes only
                }).each(function() {
                    this.nodeValue = this.nodeValue.replace(/\s*::\s*/, ' ');
                });
                // Also remove :: from <p> children (happens with code blocks)
                // The <a> might be inside the <p>, so match :: before <a>
                $this.children('p').each(function() {
                    var html = $(this).html();
                    if (html.indexOf('::') !== -1) {
                        $(this).html(html.replace(/\s*::\s*(<a)/, '$1'));
                    }
                });
            }
          } // end function after each
        )// end each
    // hide(), not toggle(): the comment always said close, but a flip lands on
    // whichever state the lists were left in, so the TOC came up open or shut
    // depending on what ran before it. Closed, with the arrows saying so — they
    // are built reading [-], which meant open while the list was hidden.
    $('ul.lev1').hide();                            //  close level 1 in TOC
    $('ul.lev2').hide();                            //  close level 2 in TOC
    $('#toc span.li_h1, #toc span.li_h2, #toc span.li_h3').text(' [+]');

//===================================================================== Wrap group labels for click targeting (after :: removal)

    // Wrap direct text nodes of video group parents in inline-block span
    // so clicks on empty space don't trigger "select all videos"
    $('.strapvid_inside').each(function(){
        $(this).contents().filter(function(){ return this.nodeType === 3 && $.trim(this.nodeValue) !== '' }).wrap('<span class="vid-group-label"/>')
    })
    $('li').each(function(){
        if ($(this).find('.vid-lazy-list').length && !$(this).hasClass('vid-lazy-item')) {
            $(this).contents().filter(function(){ return this.nodeType === 3 && $.trim(this.nodeValue) !== '' }).wrap('<span class="vid-group-label"/>')
        }
    })
...
//===================================================================== key actions
    ...
    */
    ...
    $(document).keydown(function(event){
        ...
        var list_wind = ['syntax', 'keys', 'carr'] // list of windows
...
//===================================================================== // taking href with blanks
    ...
    var maketag = function(self, deb, end, select){ // makes the tags for pdfs and videos
        ...
        var newLink = $('<a/>').text(text).append($('<a/>').append($('<span/>').text( " [-]").addClass(select)))
                 .css({'color': debend[select]['color']})
                 .attr('title', self.attr('title'));     // Adding tooltip

        // Add metadata for PDF links to enable annotations
        if (select === '§§') {
            newLink.addClass('pdf-link')
                   .attr('data-pdf-path', self.attr('href'));
        }

        newLink.insertBefore(self);
        return tag
    }
    ...
    $("a").each(function(){  // Deals with videos and pdfs and iframes
         for (i in sel){
             if($(this).text().search(sel[i]) != -1){
                   // For PDFs (§§), don't create <object> - we'll use PDF.js viewer instead
                   if (sel[i] === '§§') {
                       var href = $(this).attr('href');
                       // Check if it's a PDF (not an EPUB)
                       if (href && href.toLowerCase().endsWith('.pdf')) {
                           // For PDFs only - use PDF.js viewer
                           var text = $(this).text().replace(sel[i],'')
                           var newLink = $('<a/>').text(text).append($('<a/>').append($('<span/>').text( " [-]").addClass(sel[i])))
                                    .css({'color': debend[sel[i]]['color']})
                                    .attr('title', $(this).attr('title'))
                                    .addClass('pdf-link')
                                    .attr('data-pdf-path', href);
                           $(this).replaceWith(newLink);
                       } else {
                           // For EPUBs and other documents - use original object behavior
                           var obj = maketag($(this), debend[sel[i]]['deb'], debend[sel[i]]['end'], sel[i]) // makes the tag.
                           $(this).replaceWith(obj)
                           obj.toggle() // hide
                       }
                   } else {
                       // For videos (;;), use original behavior with <object>
                       var obj = maketag($(this), debend[sel[i]]['deb'], debend[sel[i]]['end'], sel[i]) // makes the tag.
                       $(this).replaceWith(obj)
                       obj.toggle() // hide
                   }
    ...
    */
    ...
    $("a").click(function (evt) {
        ...
        if( evtc == ';;' | evtc == '§§' ) {      // toggle
            ...
            // If this is a PDF link, open with PDF.js viewer instead of object
            if (evtc == '§§' && $(this).hasClass('pdf-link')) {
                evt.preventDefault(); // Prevent default toggle behavior
                evt.stopPropagation(); // Stop event from bubbling up

                var pdfPath = $(this).attr('data-pdf-path');

                // DO NOT activate annotation mode automatically
                // User must use Alt+A to activate annotations manually

                // Set the current file for annotations (but don't activate)
                if (window.annotationSystem) {
                    window.annotationSystem.currentFile = pdfPath;
                    window.annotationSystem.fileType = 'pdf';
                }

                // Open PDF in PDF.js viewer modal
                if (window.openPDFViewer) {
                    window.openPDFViewer(pdfPath);

                    // Wait for PDF to load, then setup annotation handlers
                    setTimeout(function() {
                        console.log('PDF opened, setting up annotations...');

                        // Load annotations for this PDF
                        if (window.socket) {
                            window.socket.emit('get_annotations', pdfPath);
                        }

                        // Use the global setupPDFJSAnnotationHandlers if available
                        if (window.setupPDFJSAnnotationHandlers) {
                            console.log('Calling setupPDFJSAnnotationHandlers...');
                            window.setupPDFJSAnnotationHandlers();
                        } else {
                            console.error('setupPDFJSAnnotationHandlers not found');
                        }

                        // Also trigger reloadPDFAnnotations to display existing annotations
                ...
                // For EPUBs/DJVUs (§§ without pdf-link class)
                // Just toggle - openPDFViewer now checks file extension automatically
                $(this).parent().next().toggle();
            } else {
                // For videos (;;), use original toggle behavior
                $(this).parent().next().toggle();
            }
            } // end if event
        });// end click
...
//=====================================================================  Mini Table
    ...
    /*
    Insert an inline editable table with $tab tableId
    Data persisted in static/tables.yaml via socket.
    */
    ...
    $("p, li").each(function(){
         ...
         if (text.match(/^\$tab\s+/)){
                 var tableName = text.split(/\s+/)[1].trim()
                 var tableId = tableName
                 if (_usedTableIds[tableId]){
                     var n = 2
                     while (_usedTableIds[tableId + '_' + n]) n++
                     tableId = tableId + '_' + n
                 }
                 _usedTableIds[tableId] = true
                 var container = $('<div/>').attr('id', 'tab_' + tableId).addClass('mini-table-widget')
                 $(this).replaceWith(container)
                 initMiniTable(tableId, tableName)
            }
        })
...
//=====================================================================  Toggle titles and list
    ...
    $("a").click(function (evt) {                   // toggle for H1, H2, H3 etc..( has to be after hide lists)
        ...
        if(evtc == 'li_h1' | evtc == 'li_h2' | evtc == 'li_h3' | evtc == '::') { // open close list on click
            evt.stopPropagation();
            evt.preventDefault();
            // Find the <ul> to toggle - could be next sibling or sibling of parent <p>
            var $next = $(this).next();
            if (!$next.is('ul')) {
                $next = $(this).parent().next(); // <a> is inside <p>, get sibling of <p>
            }
            $next.toggle();
            // Keep the arrow in step. Only the :: case used to be updated, so a
            // TOC arrow read [-] for ever whatever its list was doing.
            if(evtc == 'li_h1' || evtc == 'li_h2' || evtc == 'li_h3') {
                $(evt.target).text($next.is(':visible') ? ' [-]' : ' [+]');
            }
            // Toggle the indicator for :: class
            if(evtc == '::') {
                var $indicator = $(this).find('span.\\:\\:');
                var currentText = $indicator.text().trim();
                $indicator.text(currentText === '[+]' ? ' [-]' : ' [+]');
                // deep fold mode: toggle all :: descendants too
                if (deep_fold_mode){
                    var action = currentText === '[+]' ? 'open' : 'close'
                    $next.find('span.\\:\\:').each(function(){
                        var $da = $(this).closest('a')
                        var $dul = $da.next('ul')
                        if (!$dul.length) $dul = $da.parent().next('ul')
                        if ($dul.length){
                            if (action === 'open') { $dul.show() } else { $dul.hide() }
    ...
    */
     ...
     $("img").each(function(){               // retrieve the caption and insert it under the image.
             ...
             if ($(this).attr('alt').match(reg_caption)){
               ...
               //alert(capt)
               var captid = capt.replace(/\s+/g, '').replace(/\//g, '_') + Math.floor((Math.random() * 1000) + 1)
               //alert('captid is '+ captid)
...
//=====================================================================
 ...
 $('a').each(function(){
        ...
        if  ($(this).text().match('%%')){       // insert local video, works with Chrome
            ...
            var track = $('<track/>', {'label': 'english', 'src': vid_url.replace('.mp4', '.vtt'), 'kind': 'subtitles', 'srclang': 'es' })
            vid.append(track)
            var div = $('<div/>').css({'text-align':'center'})
...
//===================================================================== Alt+Q deep fold mode

    var deep_fold_mode = false
...
//===================================================================== Close all the lists
    ...
    /*
    Alt + Q : toggle deep fold mode.
    When active, clicking [+]/[-] unfolds/folds entire hierarchy recursively.
    */

    if(event.altKey && event.keyCode === 81){
                // toggle deep fold mode
                deep_fold_mode = !deep_fold_mode
                if (deep_fold_mode){
    ...
    */
     ...
     $("a").each(function(){                             // insert pdf from folder
         ...
         if (txt.match(reg_folder)){                     // check <<
            ...
            // alert(jsoname)
            pdf_folder = $('<div/>').addClass('lazy-pdf-folder')  // make a div for inserting the pdfs
            ...
            $.getJSON(jsoname, function(result){
                for (i in result){                      // for each address in the json..
                    // Lazy loading: create placeholder instead of loading PDF immediately
                    var pdfPath = result[i]
                    var pdfName = pdfPath.split('/').pop().replace('.pdf', '')
                    var lazyContainer = $('<div/>').addClass('lazy-pdf-item')
                    var titleDiv = $('<div/>').addClass('lazy-pdf-title').text(pdfName)
                    var loadBtn = $('<button/>')
                        .addClass('lazy-pdf-load-btn')
                        .text('Charger le PDF')
                        .attr('data-pdf-path', pdfPath)
                    var pdfContainer = $('<div/>').addClass('lazy-pdf-container').hide()

                    // Click handler to load PDF on demand
                    loadBtn.on('click', function(){
                        var btn = $(this)
                        var path = btn.attr('data-pdf-path')
                        var container = btn.siblings('.lazy-pdf-container')

                        if (container.is(':empty')) {
                            // First click: load the PDF
                            btn.text('Chargement...')
                            var obj = $('<object/>').attr('data', path)
                    ...
                    lazyContainer.append(titleDiv).append(loadBtn).append(pdfContainer)
                    pdf_folder.append(lazyContainer)
                } // end for
            }) // end get json
         }// end if
     }) // end each
...
//===================================================================== Nice scroller
    ...
    */
    ...
    // Consolidated scroll handler for all scroll-related functions
    $(window).scroll(function(){
        ...
        // Show when not at bottom (more than 100px from bottom)
    ...
    // Scroll
    lscrolls = ['syntax', 'toc', 'keys', 'carr', 'params']
    for (i in lscrolls){
...
function initMiniTable(tableId, tableName){
    tableName = tableName || tableId
    // wait for socket to be available (created after maketoc)
    function waitForSocket(cb){
        if (window.socket) return cb(window.socket)
        var check = setInterval(function(){
            if (window.socket){ clearInterval(check); cb(window.socket) }
        }, 50)
    }
    waitForSocket(function(sock){
    var $container = $('#tab_' + tableId)
    var tableData = [['', ''], ['', '']] // default 2x2
    var tableSettings = { fontSize: 12, headerFontSize: 14, titleFontSize: 12, headerStyle: 'bold', colStats: {} }

    function applyHeaderStyle($td){
        var style = tableSettings.headerStyle
        if (style === undefined) style = tableSettings.headerBold === false ? 'normal' : 'bold'
        $td.css('font-weight', style === 'bold' ? 'bold' : 'normal')
          .css('font-style', style === 'italic' ? 'italic' : 'normal')
    }
    var saveTimer = null
    var selection = {} // key "r_c" -> true
    var lastClicked = null // {r, c}
    var rowHighlight = {} // row index -> true

    function applyRowHighlight(){
        $container.find('.mini-table tr').each(function(idx){
            var $cells = $(this).find('td')
            if (rowHighlight[idx]){
                $cells.css({'border-top': '2px solid #2a7ae4', 'border-bottom': '2px solid #2a7ae4'})
                $cells.first().css('border-left', '2px solid #2a7ae4')
                $cells.last().css('border-right', '2px solid #2a7ae4')
            } else {
                $cells.css({'border-top': '', 'border-bottom': '', 'border-left': '', 'border-right': ''})
            }
        })
    }

    function selKey(r, c){ return r + '_' + c }
    function isSelectable(r, c){
        return r > 0 && c > 0 && r < tableData.length - 1
    }
    function applySelectionStyles(){
        $container.find('.mini-table td[data-r]').each(function(){
            var r = parseInt($(this).attr('data-r'))
            var c = parseInt($(this).attr('data-c'))
            if (selection[selKey(r, c)]){
                $(this).css('background-color', '#d8ecff')
            } else {
    ...
    function computeColStats(colIdx, stats){
        var lastRow = tableData.length - 1
        var vals = []
        var selectedRows = []
        for (var k in selection){
            var p = k.split('_')
            if (parseInt(p[1]) === colIdx) selectedRows.push(parseInt(p[0]))
        }
        if (selectedRows.length > 0){
            selectedRows.forEach(function(r){
                var v = parseFloat(tableData[r][colIdx])
        ...
        var parts = []
        if (stats.mean) parts.push('μ=' + (+mean.toFixed(4)))
        if (stats.std){
            var variance = vals.reduce(function(a,b){ return a + (b - mean) * (b - mean) }, 0) / n
            parts.push('σ=' + (+Math.sqrt(variance).toFixed(4)))
        }
        return parts.join(' ')
    }

    function recomputeAllStats(){
        if (!tableSettings.colStats) return
        var lastRow = tableData.length - 1
        if (lastRow < 1) return
        var changed = false
        for (var cKey in tableSettings.colStats){
            var colIdx = parseInt(cKey)
            var stats = tableSettings.colStats[cKey]
            if (!stats || (!stats.mean && !stats.std)) continue
            if (colIdx >= (tableData[lastRow] || []).length) continue
            var val = computeColStats(colIdx, stats)
            if (tableData[lastRow][colIdx] !== val){
                tableData[lastRow][colIdx] = val
                var $cell = $container.find('.mini-table td[data-r="' + lastRow + '"][data-c="' + colIdx + '"]')
                if ($cell.length) $cell.text(val)
                changed = true
            }
        }
    ...
    function showColStatsPopup($cell, colIdx){
        $('.mini-table-stats-popup').remove()
        var offset = $cell.offset()
        var $popup = $('<div/>').addClass('mini-table-settings-popup mini-table-stats-popup').css({
            'position': 'absolute',
            'top': (offset.top + $cell.outerHeight() + 2) + 'px',
            'left': offset.left + 'px'
        })
        var $close = $('<span/>').text('\u2715').addClass('mini-table-popup-close')
            .click(function(){ $popup.remove() })
        var statsFs = Math.max(8, (tableSettings.headerFontSize || 14) - 2) + 'px'
        var current = (tableSettings.colStats && tableSettings.colStats[colIdx]) || {}
        var $meanLbl = $('<label/>').css({'display': 'block', 'font-size': statsFs, 'margin-bottom': '6px', 'cursor': 'pointer'})
        var $meanCb = $('<input/>').attr('type', 'checkbox').prop('checked', !!current.mean).css('margin-right', '6px')
        $meanLbl.append($meanCb).append('mean')
        var $stdLbl = $('<label/>').css({'display': 'block', 'font-size': statsFs, 'margin-bottom': '8px', 'cursor': 'pointer'})
        var $stdCb = $('<input/>').attr('type', 'checkbox').prop('checked', !!current.std).css('margin-right', '6px')
        $stdLbl.append($stdCb).append('std')
        var $apply = $('<button/>').text('apply').css({'font-size': statsFs, 'padding': '3px 10px', 'cursor': 'pointer'})
        $apply.click(function(){
            var stats = { mean: $meanCb.is(':checked'), std: $stdCb.is(':checked') }
            if (!tableSettings.colStats) tableSettings.colStats = {}
            if (!stats.mean && !stats.std){
                delete tableSettings.colStats[colIdx]
                var lastRow = tableData.length - 1
                tableData[lastRow][colIdx] = ''
            } else {
                tableSettings.colStats[colIdx] = stats
                var lastRow = tableData.length - 1
                tableData[lastRow][colIdx] = computeColStats(colIdx, stats)
            }
            render(); save()
            $popup.remove()
        })
        $popup.append($close).append($meanLbl).append($stdLbl).append($apply)
        $('body').append($popup)
        setTimeout(function(){
            $(document).one('click.minitablestats', function(){ $popup.remove() })
            $popup.on('click', function(e){ e.stopPropagation() })
        }, 0)
    }

    function resizeTable(newRows, newCols){
        newRows = Math.max(1, parseInt(newRows) || 1)
        newCols = Math.max(1, parseInt(newCols) || 1)
        // rows
        while (tableData.length < newRows){
            var cols = tableData[0] ? tableData[0].length : newCols
            var row = []
            for (var j = 0; j < cols; j++) row.push('')
            tableData.push(row)
        }
        while (tableData.length > newRows) tableData.pop()
        // cols
        for (var i = 0; i < tableData.length; i++){
            while (tableData[i].length < newCols) tableData[i].push('')
            while (tableData[i].length > newCols) tableData[i].pop()
        }
    }
    ...
    function render(){
        ...
        // title + settings
        var $header = $('<div/>').css({'display': 'flex', 'align-items': 'center', 'margin-bottom': '8px'})
        var $title = $('<span/>').addClass('mini-table-title').css({'margin-bottom': '0', 'font-size': (tableSettings.titleFontSize || 12) + 'px'}).text(tableName)
        var $settingsBtn = $('<img/>').attr('src', 'capicon/svg/007.svg')
            .attr('title', 'Table settings')
            .addClass('mini-table-settings-btn')
            .css({'width': '16px', 'height': '16px', 'cursor': 'pointer', 'margin-left': '8px', 'opacity': '0', 'transition': 'opacity 0.15s'})
        $settingsBtn.on('mouseenter', function(){ $(this).css('opacity', '1') })
                    .on('mouseleave', function(){
                        if (!$('.mini-table-settings-popup').length) $(this).css('opacity', '0')
                    })

        $settingsBtn.click(function(e){
            e.stopPropagation()
            // toggle: close if already open
            var $existing = $('.mini-table-settings-popup').not('.mini-table-stats-popup')
            if ($existing.length){ $existing.remove(); return }
            $('.mini-table-settings-popup').remove()
            var offset = $(this).offset()
            var $popup = $('<div/>').addClass('mini-table-settings-popup').css({
                'position': 'absolute',
            ...
            var $close = $('<span/>').text('\u2715').addClass('mini-table-popup-close')
                .click(function(){ $popup.remove() })
            var $popupTitle = $('<div/>').text('Table settings').css({
                'font-size': '12px', 'font-weight': 'bold', 'color': '#333',
                'margin-bottom': '10px', 'padding-bottom': '6px', 'border-bottom': '1px solid #eee'
            })

            var inputCss = {'width': '60px', 'font-size': '11px'}
            function makeRow(labelText, $input){
                var $row = $('<div/>').css({'display': 'flex', 'align-items': 'center', 'justify-content': 'space-between', 'gap': '10px', 'margin-bottom': '8px'})
                var $lbl = $('<span/>').text(labelText).css({'font-size': '11px', 'color': '#666'})
                $row.append($lbl).append($input)
                return $row
            }

            // Title size
            var $titleInput = $('<input/>').attr('type', 'number').attr('min', '8').attr('max', '40').val(tableSettings.titleFontSize || 12).css(inputCss)
            $titleInput.on('input', function(){
                var v = parseInt($(this).val()); if (!v) return
                tableSettings.titleFontSize = v
                $container.find('.mini-table-title').css('font-size', v + 'px')
                debounceSave()
            })

            // Body text size
            var $sizeInput = $('<input/>').attr('type', 'number').attr('min', '8').attr('max', '40').val(tableSettings.fontSize).css(inputCss)
            $sizeInput.on('input', function(){
                var v = parseInt($(this).val()); if (!v) return
                tableSettings.fontSize = v
                $container.find('.mini-table tr').each(function(idx){
                    if (idx > 0) $(this).find('td').css('font-size', v + 'px')
                })
                debounceSave()
            })

            // Header font size (first row)
            var $headerInput = $('<input/>').attr('type', 'number').attr('min', '8').attr('max', '40').val(tableSettings.headerFontSize).css(inputCss)
            $headerInput.on('input', function(){
                var v = parseInt($(this).val()); if (!v) return
                tableSettings.headerFontSize = v
                $container.find('.mini-table tr').first().find('td').css('font-size', v + 'px')
                debounceSave()
            })

            // Rows count
            var $rowsInput = $('<input/>').attr('type', 'number').attr('min', '1').attr('max', '200').val(tableData.length).css(inputCss)
            $rowsInput.on('change', function(){
                resizeTable(parseInt($(this).val()), tableData[0].length)
                render(); save()
            })

            // Cols count
            var $colsInput = $('<input/>').attr('type', 'number').attr('min', '1').attr('max', '50').val(tableData[0].length).css(inputCss)
            $colsInput.on('change', function(){
                resizeTable(tableData.length, parseInt($(this).val()))
                render(); save()
            })

            // Header style
            var currentStyle = tableSettings.headerStyle
            if (currentStyle === undefined) currentStyle = tableSettings.headerBold === false ? 'normal' : 'bold'
            var $styleInput = $('<select/>').css(inputCss)
            ;['bold', 'italic', 'normal'].forEach(function(opt){
                var $o = $('<option/>').val(opt).text(opt)
                if (opt === currentStyle) $o.prop('selected', true)
                $styleInput.append($o)
            })
            $styleInput.on('change', function(){
                tableSettings.headerStyle = $(this).val()
                delete tableSettings.headerBold
                $container.find('.mini-table tr').first().find('td').each(function(){ applyHeaderStyle($(this)) })
                debounceSave()
            })

            $popup.append($close)
                  .append($popupTitle)
                  .append(makeRow('Title size', $titleInput))
                  .append(makeRow('Body text size', $sizeInput))
                  .append(makeRow('Header text size', $headerInput))
                  .append(makeRow('Rows', $rowsInput))
                  .append(makeRow('Columns', $colsInput))
                  .append(makeRow('Header style', $styleInput))
            $('body').append($popup)
            // close on click outside
            setTimeout(function(){
                $(document).one('click.minitablesettings', function(){ $popup.remove() })
                $popup.on('click', function(e){ e.stopPropagation() })
            }, 0)
        })

        // CSV export button
        var $csvBtn = $('<img/>').attr('src', 'capicon/svg/226.svg')
            .attr('title', 'Export as CSV')
            .css({'width': '16px', 'height': '16px', 'cursor': 'pointer', 'margin-left': '6px', 'opacity': '0', 'transition': 'opacity 0.15s'})
        $csvBtn.on('mouseenter', function(){ $(this).css('opacity', '1') })
               .on('mouseleave', function(){ $(this).css('opacity', '0') })
        ...
        $header.append($title).append($settingsBtn).append($csvBtn)
        $container.append($header)

        // layout: table + column buttons on the right
        var $row = $('<div/>').css({'display': 'flex', 'align-items': 'center'})

        // table
        var $table = $('<table/>').addClass('mini-table')
        for (var r = 0; r < tableData.length; r++){
            var $tr = $('<tr/>')
            for (var c = 0; c < tableData[r].length; c++){
                var fs = (r === 0 ? tableSettings.headerFontSize
                         : r === tableData.length - 1 ? Math.max(8, tableSettings.fontSize - 1)
                         : tableSettings.fontSize)
                // auto-number first column for data rows (row 1 onwards)
                if (c === 0 && r > 0){
                    var $td = $('<td/>').text(r)
                                        .attr('data-rownum', r)
                                        .css({'font-size': fs + 'px', 'text-align': 'center',
                                              'background-color': '#f5f5f5', 'color': '#888',
                                              'user-select': 'none', 'cursor': 'pointer'})
                    $td.on('dblclick', function(e){
                        e.preventDefault()
                        var row = parseInt($(this).attr('data-rownum'))
                        if (rowHighlight[row]) delete rowHighlight[row]; else rowHighlight[row] = true
                        applyRowHighlight()
                    })
                    $tr.append($td)
                    continue
                ...
                var $td = $('<td/>').attr('contenteditable', 'true')
                                    ...
                                    .css('font-size', fs + 'px')
                if (r === 0) applyHeaderStyle($td)
                $td.on('input', function(){
                    var row = parseInt($(this).attr('data-r'))
                    var col = parseInt($(this).attr('data-c'))
                    tableData[row][col] = $(this).text()
                    if (row < tableData.length - 1) recomputeAllStats()
                    debounceSave()
                })
                $td.on('mousedown', function(e){
                    ...
                    if (e.ctrlKey || e.metaKey){
                        e.preventDefault()
                        var k = selKey(row, col)
                        if (selection[k]) delete selection[k]; else selection[k] = true
                        lastClicked = {r: row, c: col}
                        applySelectionStyles()
                    } else if (e.shiftKey){
                        e.preventDefault()
                        var r0 = lastClicked ? lastClicked.r : row
                        var c0 = lastClicked ? lastClicked.c : col
                        var rMin = Math.min(r0, row), rMax = Math.max(r0, row)
                        var cMin = Math.min(c0, col), cMax = Math.max(c0, col)
                        for (var rr = rMin; rr <= rMax; rr++){
                            for (var cc = cMin; cc <= cMax; cc++){
                                if (isSelectable(rr, cc)) selection[selKey(rr, cc)] = true
                            }
                        }
                        applySelectionStyles()
                    } else {
                ...
                if (r === tableData.length - 1 && r > 0 && c > 0){
                    $td.on('dblclick', function(e){
                        e.preventDefault()
                        var col = parseInt($(this).attr('data-c'))
                        showColStatsPopup($(this), col)
                    })
                }
                $tr.append($td)
            }
        ...
        // column +/- buttons (right of table, vertical)
        var $colBtns = $('<div/>').addClass('mini-table-col-btns').css({'display': 'flex', 'flex-direction': 'column', 'margin-left': '5px', 'gap': '3px', 'align-self': 'flex-start', 'padding': '5px'})
        $colBtns.append($('<button/>').addClass('mini-table-btn-inline').attr('title', 'Remove last column').text('\u2212').click(function(){
            if (tableData[0] && tableData[0].length > 1){
                for (var i = 0; i < tableData.length; i++) tableData[i].pop()
                render(); save()
            }
        }))
        $colBtns.append($('<button/>').addClass('mini-table-btn-inline').attr('title', 'Add a column').text('+').click(function(){
            for (var i = 0; i < tableData.length; i++) tableData[i].push('')
            render(); save()
        }))
        $row.append($colBtns)
        $container.append($row)

        // row +/- buttons as last row of table, aligned to last column
        var numCols = tableData[0] ? tableData[0].length : 2
        var $trBtns = $('<tr/>').addClass('mini-table-btns-row')
        if (numCols > 1){
            $trBtns.append($('<td/>').attr('colspan', numCols - 1).css({'border': 'none', 'padding': '0', 'line-height': '0', 'height': '0'}).html('&nbsp;').css('font-size', '0'))
        }
        var $tdBtns = $('<td/>').addClass('mini-table-row-btns').css({'border': 'none', 'padding': '5px 0', 'text-align': 'right'})
        $tdBtns.append($('<button/>').addClass('mini-table-btn-inline').attr('title', 'Remove last row').text('\u2212').click(function(){
            if (tableData.length > 1){
                tableData.pop()
                render(); save()
            }
        }))
        $tdBtns.append($('<button/>').addClass('mini-table-btn-inline').css('margin-left', '3px').attr('title', 'Add a row').text('+').click(function(){
            var cols = tableData[0] ? tableData[0].length : 2
            var row = []
            for (var j = 0; j < cols; j++) row.push('')
            tableData.push(row)
            render(); save()
        }))
        $trBtns.append($tdBtns)
        $table.append($trBtns)
        applySelectionStyles()
        applyRowHighlight()
    }
    ...
    function save(){
        sock.emit('save_table_data', JSON.stringify({ id: tableId, data: tableData, settings: tableSettings }))
    }
    ...
    $container.on('dragover', function(e){
        e.preventDefault()
        e.stopPropagation()
        $(this).css('outline', '2px dashed #2a7ae4')
    })
    $container.on('dragleave drop', function(){
        $(this).css('outline', '')
    })
    $container.on('drop', function(e){
        ...
        reader.onload = function(ev){
            ...
            // Replace table data entirely
            tableData = parsed
            tableSettings.colStats = {}
            selection = {}
            lastClicked = null
            rowHighlight = {}
            render()
    ...
    sock.emit('get_table_data', tableId)
    sock.on('table_data_' + tableId, function(json){
        var data = JSON.parse(json)
        if (data && data.data) {
            tableData = data.data
            if (data.settings) tableSettings = data.settings
        } else if (data && data.length) {
            tableData = data // backward compat: old format was just the array
        }
        render()
    })
    }) // end waitForSocket
