...
function initMiniTable(tableId){
    ...
    function showColStatsPopup($cell, colIdx){
        ...
        var $meanLbl = $('<label/>').css({'display': 'block', 'font-size': '11px', 'margin-bottom': '6px', 'cursor': 'pointer'})
        var $meanCb = $('<input/>').attr('type', 'checkbox').css('margin-right', '6px')
        $meanLbl.append($meanCb).append('mean')
        var $stdLbl = $('<label/>').css({'display': 'block', 'font-size': '11px', 'margin-bottom': '8px', 'cursor': 'pointer'})
        var $stdCb = $('<input/>').attr('type', 'checkbox').css('margin-right', '6px')
        $stdLbl.append($stdCb).append('std')
        var $apply = $('<button/>').text('apply').css({'font-size': '11px', 'padding': '3px 10px', 'cursor': 'pointer'})
    ...
    function render(){
        ...
        $container.off('mouseenter.minitablebtn mouseleave.minitablebtn')
            .on('mouseenter.minitablebtn', function(){ $container.find('.mini-table-settings-btn').css('opacity', '0.6') })
            .on('mouseleave.minitablebtn', function(){
                if (!$('.mini-table-settings-popup').length){
                    $container.find('.mini-table-settings-btn').css('opacity', '0')
                }
            })
        $settingsBtn.on('mouseenter', function(){ $(this).css('opacity', '1') })
                    .on('mouseleave', function(){ $(this).css('opacity', '0.6') })
