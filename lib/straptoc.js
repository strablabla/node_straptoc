...
function initMiniTable(tableId){
    ...
    var tableSettings = { fontSize: 14, headerFontSize: 14 }
    var saveTimer = null
    ...
    function render(){
        ...
        var $settingsBtn = $('<span/>').text('\u2699')
            .addClass('mini-table-settings-btn')
            .css({'font-size': '14px', 'cursor': 'pointer', 'margin-left': '8px', 'opacity': '0.4'})
        ...
        $settingsBtn.click(function(e){
            ...
            // Font size (body)
            var $sizeLabel = $('<div/>').text('Taille du texte').css({'font-size': '11px', 'margin-bottom': '5px', 'color': '#666'})
            var $sizeRow = $('<div/>').css({'display': 'flex', 'align-items': 'center', 'gap': '8px', 'margin-bottom': '10px'})
            var $sizeInput = $('<input/>').attr('type', 'range').attr('min', '10').attr('max', '24').val(tableSettings.fontSize).css('width', '100px')
            var $sizeVal = $('<span/>').text(tableSettings.fontSize + 'px').css('font-size', '11px')
            $sizeInput.on('input', function(){
                tableSettings.fontSize = parseInt($(this).val())
                $sizeVal.text(tableSettings.fontSize + 'px')
                $container.find('.mini-table tr').each(function(idx){
                    if (idx > 0) $(this).find('td').css('font-size', tableSettings.fontSize + 'px')
                })
                debounceSave()
            })
            $sizeRow.append($sizeInput).append($sizeVal)

            // Font size (header = first row)
            var $headerLabel = $('<div/>').text('Taille noms de colonnes').css({'font-size': '11px', 'margin-bottom': '5px', 'color': '#666'})
            var $headerRow = $('<div/>').css({'display': 'flex', 'align-items': 'center', 'gap': '8px'})
            var $headerInput = $('<input/>').attr('type', 'range').attr('min', '10').attr('max', '24').val(tableSettings.headerFontSize).css('width', '100px')
            var $headerVal = $('<span/>').text(tableSettings.headerFontSize + 'px').css('font-size', '11px')
            $headerInput.on('input', function(){
                tableSettings.headerFontSize = parseInt($(this).val())
                $headerVal.text(tableSettings.headerFontSize + 'px')
                $container.find('.mini-table tr').first().find('td').css('font-size', tableSettings.headerFontSize + 'px')
                debounceSave()
            })
            $headerRow.append($headerInput).append($headerVal)

            $popup.append($close).append($sizeLabel).append($sizeRow).append($headerLabel).append($headerRow)
            $('body').append($popup)
