...
// Serialize read-modify-write on static/config.yaml.
// Why: closeModal() fires update_color_tags + update_aliases on the same tick;
// their handlers race on the file and leave stale bytes at the end, producing
// YAML syntax errors. Chaining each op through configQueue guarantees atomicity.
var configQueue = Promise.resolve();
function updateConfigYaml(modifier) {
    var next = configQueue.then(function() {
        return new Promise(function(resolve, reject) {
            fs.readFile('static/config.yaml', 'utf8', function(err, text) {
                if (err) return reject(err);
                var config;
                try { config = yaml.load(text); }
                catch(e) { return reject(e); }
                try { modifier(config); }
                catch(e) { return reject(e); }
                var newYaml = yaml.dump(config, { indent: 2, lineWidth: -1, noRefs: true });
    ...
    configQueue = next.catch(function() {});
    return next;
}
...
exports.handle = function(io,socket){
      ...
      socket.on('update_color_tags', function(newColorTagsJson) {
          ...
          updateConfigYaml(function(fullConfig) {
              fullConfig.color_tags = JSON.parse(newColorTagsJson);
          }).then(function() {
    ...
    socket.on('get_git_update_interval', function() {
        fs.readFile('static/config.yaml', 'utf8', function (err, text) {
            if (err) {
                socket.emit('git_update_interval_data', { minutes: 5 });
                return;
            }
            try {
                var config = yaml.load(text);
                var minutes = config.git_update_interval || 5;
                socket.emit('git_update_interval_data', { minutes: minutes });
            } catch (e) {
                socket.emit('git_update_interval_data', { minutes: 5 });
            }
        });
    ...
    socket.on('save_git_update_interval', function(minutes) {
        fs.readFile('static/config.yaml', 'utf8', function (err, text) {
            if (err) {
                socket.emit('git_update_interval_updated', { success: false, error: 'Could not read config file' });
                return;
            }
            try {
                var config = yaml.load(text);
                config.git_update_interval = minutes;
                var yamlStr = yaml.dump(config);
                fs.writeFile('static/config.yaml', yamlStr, 'utf8', function(err) {
                    if (err) {
                        socket.emit('git_update_interval_updated', { success: false, error: 'Could not write config file' });
                        return;
                    }
                    socket.emit('git_update_interval_updated', { success: true, minutes: minutes });
                });
            } catch (e) {
                socket.emit('git_update_interval_updated', { success: false, error: 'Error parsing config file' });
            }
        });
    });
    ...
    socket.on('git_pull_restart', function() {
        ...
        exec('git pull', { cwd: process.cwd() }, function(err, stdout, stderr) {
            ...
            var pullOut = stdout.trim();

            // Run npm install to pick up any new/updated packages (no-op if already in sync)
            exec('npm install --no-audit --no-fund', { cwd: process.cwd() }, function(npmErr, npmOut, npmErrOut) {
                console.log('npm install stdout:', npmOut);
                if (npmErrOut) console.log('npm install stderr:', npmErrOut);

                if (npmErr) {
                    console.error('npm install failed:', npmErr.message);
                    socket.emit('git_pull_result', { success: false, message: 'git pull OK but npm install failed: ' + npmErr.message });
                    return;
                }

                socket.emit('git_pull_result', { success: true, message: pullOut });
    ...
    socket.on('save_addr_saved', function(absolutePath) {
        ...
        try {
            ...
            if (!stat.isDirectory()) {
                socket.emit('addr_saved_updated', JSON.stringify({ success: false, error: 'Path exists but is not a directory: ' + absolutePath }));
                return;
            }
        } catch (e) {
            socket.emit('addr_saved_updated', JSON.stringify({ success: false, error: 'Directory does not exist: ' + absolutePath }));
            return;
        }
