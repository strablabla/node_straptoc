      ...
      */
      ...
      try {
        ...
        for (const [key, val] of Object.entries(flat_dic)) {
          ...
          // redirect:false — several of these roots are big shared drives, and a
          // folder in one of them that happens to carry a page's name (e.g.
          // /mnt/sdata/utile) would otherwise win: serve-static answers the bare
          // /utile with a 301 to /utile/, and the page's asset URLs, being
          // relative, then resolve one level too deep. Without this, the request
          // falls through to the page route instead.
          app.use(express.static(stat_addr, { redirect: false })); // serve files from directory
        }
      } catch (e) {
...
exports.feed_page = function(app, page){
  var model = make_html_around_md(page.name + '_md.html');
  app.get(page.path, function (req, res) {
    // A 301 to /page/ cached from before the redirect:false fix above can still
    // land a browser here one level too deep. Bounce it back to /page, or every
    // relative asset URL in the rendered page 404s and nothing gets built.
    if (page.path !== '/' && req.path === page.path + '/') {
      return res.redirect(302, page.path);
    }
    // Load aliases from config.yaml and pass to template
    var aliases = {};
    try {
      var configText = fs.readFileSync('static/config.yaml', 'utf8');
      var config = yaml.load(configText);
      aliases = config.aliases || {};
    } catch(e) { console.error('Error loading aliases:', e); }
    var html = nunjucks.renderString(model, { aliases: JSON.stringify(aliases) });
    res.send(html);
  });
}
...
exports.aliases = function(io){
  fs.readFile('static/config.yaml', 'utf8', function (err,text) {
          if (err) { return console.log(err); }
          try {
            const config = yaml.load(text);
            const aliasesJson = JSON.stringify(config.aliases || {});
            io.sockets.emit('dic_aliases', aliasesJson)
          } catch (e) {
            console.error("Erreur lors du chargement des aliases:", e);
          }
      });
}
