...
// Reading size of the note editor, shared by every file type.
function loadNoteFontSize() {
    const settings = loadDocument().settings || {};
    const size = parseInt(settings.note_font_size, 10);
    return clampNoteFontSize(isNaN(size) ? DEFAULT_NOTE_FONT_SIZE : size);
}

function clampNoteFontSize(size) {
    return Math.min(MAX_NOTE_FONT_SIZE, Math.max(MIN_NOTE_FONT_SIZE, size));
}

function saveNoteFontSize(size) {
    const doc = loadDocument();
    doc.settings = doc.settings || {};
    doc.settings.note_font_size = clampNoteFontSize(size);
    return writeDocument(doc) ? doc.settings.note_font_size : null;
}
...
function setupAnnotationHandlers(io) {
    io.on('connection', function(socket) {
        ...
        socket.on('get_note_font_size', function() {
            socket.emit('note_font_size_loaded', loadNoteFontSize());
        });

        socket.on('set_note_font_size', function(size) {
            const saved = saveNoteFontSize(parseInt(size, 10));
            if (saved === null) return;
            // Every open viewer shares the setting, so tell them all.
            io.emit('note_font_size_loaded', saved);
        });
